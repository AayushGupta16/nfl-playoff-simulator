...
async function debug() {
    ...
    // Simulate playoff selection (basic)
    console.log('\n--- Simulating Playoff Selection ---');
    const divisions = ['North', 'South', 'East', 'West'];
    const divWinners = [];
    const wcPool = [];
    
    divisions.forEach(div => {
        const divTeams = nfc.filter(t => t.division === div);
    ...
    console.log('\nDivision Winners:');
    divWinners.forEach(t => console.log(`  ${t.name} (${t.division}): ${t.wins}-${t.losses}`));
    
    wcPool.sort((a, b) => b.wins - a.wins);
    console.log('\nWild Card Pool (top 3 make it):');
    wcPool.forEach((t, i) => console.log(`  ${i < 3 ? '✓' : ' '} ${t.name} (${t.division}): ${t.wins}-${t.losses}`));
}
